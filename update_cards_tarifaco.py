...
if post_grid_idx == -1:
    ...
    # try an alternative end string
    grid_end_str2 = '</div>\n            </section>'
    post_grid_idx = content.find(grid_end_str2, grid_start)
    if post_grid_idx == -1:
        print("Grid end completely not found")
        exit(1)
    # adjust index backward to find the </div> for the grid
    # actually post_grid_idx points to the spaces before </div>
    # let's find the closing </div> of radar-grid
...
if len(cards) > 0:
    ...
    if 'analise-g1-master.html' in cards[0] and len(cards) > 1 and 'analise-g1-master.html' in cards[1]:
        cards.pop(1)
...
<div class="md:col-span-2 lg:col-span-2 bg-white/60 dark:bg-white/5 border border-zinc-200 dark:border-white/10 backdrop-blur-md rounded-2xl overflow-hidden hover:-translate-y-2 hover:shadow-2xl hover:shadow-gold-500/5 hover:border-gold-500/30 transition-all duration-300 flex flex-col justify-between min-h-[440px] group" x-transition>
    <div class="relative w-full h-64 md:h-[22rem] overflow-hidden bg-zinc-900 border-b border-zinc-200 dark:border-white/5">
        <img src="assets/capa_tarifaco.png" alt="Miniatura Tarifaço Trump" class="w-full h-full object-cover object-top transition-all duration-500 group-hover:scale-105 brightness-[1.1] contrast-[1.02] md:brightness-100 md:contrast-100" loading="lazy" decoding="async">
    </div>
    <div class="p-6 flex-grow flex flex-col justify-between space-y-4">
        <div class="space-y-3">
            ...
            <p class="text-xs text-zinc-600 dark:text-zinc-400 font-light leading-relaxed">
                Como a mídia de esquerda criminaliza a oposição para proteger o fracasso diplomático do governo petista frente ao tarifaço americano.
            </p>
        </div>
        <div class="pt-4 border-t border-zinc-200 dark:border-white/5">
            <a href="analise-cartacapital-tarifaco.html" class="inline-flex items-center text-xs font-semibold tracking-wider text-zinc-700 hover:text-gold-600 dark:text-zinc-300 dark:hover:text-gold-400 transition-colors uppercase">
                Iniciar Autópsia &nbsp;➔
            </a>
        </div>
    </div>
